...
class GetActivityTests(APITestCase):
    ...
    def test_after_transfer(self):
        ...
        last_tx = tx_list[0]
        self.assertEqual(last_tx['senders'][0], self.balance1['address'])
        self.assertEqual(last_tx['amounts_sent'][0]['recipient'], self.balance2['address'])
        self.assertEqual(float(last_tx['amounts_sent'][0]['amount']), float(transfer_amount))